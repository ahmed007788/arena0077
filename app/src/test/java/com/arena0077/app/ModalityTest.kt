...
class ModalityTest {
    ...
    fun `fromApi returns correct modality for each api value`() {
        ...
        assertEquals(Modality.SEARCH, Modality.fromApi("search"))
        assertEquals(Modality.CODE, Modality.fromApi("code"))
    }
...
class BattleModeTest {
    ...
    fun `fromApi returns correct mode for each api value`() {
        ...
        assertEquals(BattleMode.DIRECT_BATTLE, BattleMode.fromApi("direct-battle"))
...
class MessageTest {
    ...
    @Test
    fun `user message has correct modelLabel`() {
        val msg = Message(id = "1", role = "user", content = "hello")
        assertEquals("You", msg.modelLabel)
        assertTrue(msg.isUser)
    }

    @Test
    fun `assistant position a is Model A`() {
        ...
        assertFalse(msg.isModelB)
    }

    @Test
    fun `assistant position b is Model B`() {
    ...
    @Test
    fun `assistant without position defaults to Assistant`() {
        val msg = Message(id = "1", role = "assistant", content = "hi")
        assertEquals("Assistant", msg.modelLabel)
    }
}
